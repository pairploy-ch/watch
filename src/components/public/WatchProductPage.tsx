    ...
    <div className="bg-gradient p-4 hover:bg-gray-750 transition-colors cursor-pointer"  onClick={handleClick}>
      ...
      <div className="space-y-2">
        ...
        <div className="pt-3 pb-2">
          <span className="text-white text-3xl font-bold">฿{watch.price.toLocaleString()}</span>
        </div>
      </div>
    </div>
  );
      ...
      <div className="w-72 bg-[#141519] p-6 h-fit sticky top-4">
        ...
        <div className="mb-6">
          <label className="flex items-center space-x-3 cursor-pointer hover:bg-gray-800 p-2 rounded">
            ...
            />
            <span className="text-[#E0D0B9] font-medium font-olds">New Arrivals Only</span>
          </label>
        </div>
...
const WatchProductPage: React.FC = () => {
  ...
  const filteredWatches = useMemo(() => {
    return mockWatches.filter((watch) => {
      ...
      if (selectedBrands.length > 0 && !selectedBrands.some(brand => 
        brand.toUpperCase() === watch.brand.toUpperCase()
      )) {
        return false;
      }
      ...
      // Case size filter
      if (selectedCaseSizes.length > 0 && !selectedCaseSizes.includes(watch.caseSize)) {
        return false;
      }
  ...
  }, [selectedBrands, selectedModels, selectedCaseSizes, selectedYears, priceRange, isNewArrivalOnly]);
          ...
          <div className="flex-1 mt-11">
            ...
            {displayedWatches.length === 0 && (
              <div className="text-center py-12">
                <p className="text-gray-400 text-lg">No watches found matching your filters.</p>
              </div>
            )}
